...
bot.on('callback_query', async (query) => {
    ...
    if (data === 'get_new_quote') {
        try {
            ...
            // 2. Отправляем НОВОЕ сообщение с новой цитатой и клавиатурой
            await bot.sendMessage(chatId, newQuote, {
            ...
            await bot.sendMessage(chatId, 'Произошла ошибка при получении новой цитаты.'); // Лучше тоже отправить новое
        }
        bot.answerCallbackQuery(query.id);
        return;
    }
   ...
   if (data.startsWith('savequote_category_')) {
    ...
    if (quoteText) {
        try {
            ...
            await bot.sendMessage(chatId, 'Произошла ошибка при сохранении цитаты.');
        }
