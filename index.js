...
bot.on('message', async (msg) => {
    ...
    if (messageText === 'Получить цитату') {
        try {
            ...
            const mute = await getMuteStatus(chatId);
            console.log(`[Получить цитату] Chat ID: ${chatId}, Mute: ${mute}, Action: sendMessage`); // ЛОГИРОВАНИЕ
...
bot.on('callback_query', async (query) => {
    ...
    if (data === 'get_new_quote') {
        try {
            ...
            const mute = await getMuteStatus(chatId);
            console.log(`[Получить новую цитату] Chat ID: ${chatId}, Message ID: ${messageId}, Mute: ${mute}, Action: editMessageText`); // ЛОГИРОВАНИЕ
